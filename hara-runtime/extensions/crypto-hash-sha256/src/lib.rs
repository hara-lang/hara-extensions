...
fn error(message: String) -> Value {
    Value::Record(BTreeMap::from([
        ("code".into(), Value::Keyword("crypto/digest-failed".into())),
        ("message".into(), Value::String(message)),
        ("origin".into(), Value::Keyword("wasm".into())),
        ("retryable".into(), Value::Boolean(false)),
    ]))
}
...
pub extern "C" fn hta_start(pointer: *const u8, size: usize) -> i64 {
    ...
    let event = Value::Vector(vec![Value::Integer(kind), Value::Integer(task as i64), value]);
    if let Ok(frame) = hara_hta::encode(&event) { EVENTS.with(|events| events.borrow_mut().push_back(frame)); }
    task as i64
}
