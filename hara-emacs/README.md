...
The checkout keeps Emacs Lisp source under version control and generates
`.elc` bytecode locally. Bytecode is tied to the Emacs version that compiled
it, so a fresh checkout should be used from source or installed with `make
install`, which compiles for the active Emacs.
