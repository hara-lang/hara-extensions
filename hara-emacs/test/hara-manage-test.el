...
(ert-deftest hara-manage-prefix-bindings-match-workflows ()
  ...
  (should (eq (lookup-key hara-manage-prefix-map (kbd "k"))
              #'hara-manage-cancel)))

(ert-deftest hara-manage-cancel-stops-preview-process ()
  (let ((hara-manage--process 'preview-process)
        deleted disposed)
    (cl-letf (((symbol-function 'process-live-p) (lambda (_) t))
              ...
              ((symbol-function 'hara-manage--process-output)
               (lambda (process) (setq disposed process))))
      (hara-manage-cancel))
    (should (eq deleted 'preview-process))
    (should (eq disposed 'preview-process))
    (should-not hara-manage--process)))

(ert-deftest hara-manage-cancel-refuses-write-process ()
  (let ((hara-manage--process 'write-process))
    (cl-letf (((symbol-function 'process-live-p) (lambda (_) t))
              ((symbol-function 'process-get)
               (lambda (_process property)
                 (eq property 'hara-manage-applying))))
      (should-error (hara-manage-cancel) :type 'user-error))))
...
(ert-deftest hara-manage-integrates-with-hara-mode-map ()
  ...
  (with-temp-buffer
    (hara-mode)
    (should (eq (lookup-key hara-mode-map (kbd "C-c C-m s"))
                #'hara-manage-scaffold))
    (should (eq (lookup-key hara-mode-map (kbd "C-c C-m i"))
                #'hara-manage-import))
    (should (eq (lookup-key hara-mode-map (kbd "C-c C-m p"))
                #'hara-manage-purge))
    (should (eq (lookup-key hara-mode-map (kbd "C-c C-m n"))
                #'hara-manage-incomplete))
    (should (eq (lookup-key hara-mode-map (kbd "C-c C-m d"))
                #'hara-manage-pedantic))
    (should (eq (lookup-key hara-mode-map (kbd "C-c C-m m"))
